...
from helper.mocap_util import calc_angular_vel_from_quaternion, calc_diff_from_quaternion
from helper.mocap_util import BODY_JOINTS, BODY_JOINTS_IN_DP_ORDER, JOINT_WEIGHT
from helper.mocap_util import DOF_DEF, PARAMS_KP_KD,  BODY_DEFS

class MujocoInterface(object):
    def __init__(self):
        all_mujoco_body = ['world', 'pelvis', 'femur_r', 'tibia_r', 'talus_r', 'calcn_r', 'toes_r', 'patella_r', 
                            'femur_l', 'tibia_l', 'talus_l', 'calcn_l', 'toes_l', 'patella_l', 
                            'torso', 'humerus_r', 'ulna_r', 'radius_r', 'hand_r', 'humerus_l', 'ulna_l', 'radius_l', 'hand_l']

        valid_mujoco_body = ['femur_r', 'tibia_r', 'talus_r', 'calcn_r', 'toes_r', 'patella_r', 
                             'femur_l', 'tibia_l', 'talus_l', 'calcn_l', 'toes_l', 'patella_l', 
                             'torso', 
    ...
    def calc_root_errs(self, curr_root, target_root): # including root joint
        ...
        assert len(curr_root) == 6

        err = 0.0
        err += sum(abs(curr_root[:6] - target_root[:6])*1.0 )
